...
First Crack は **外部 EC の注文を収集** し、焙煎必要量を計算して完成品在庫の出荷と接続します。  
プラットフォームの API キーは **サーバーのみ** に保存され、デスクトップアプリには再表示されません。

:::note[料金プラン]
注文・チャンネル連携は **Enterprise**（または注文連携が含まれるプラン）で利用できます。ロック画面が出る場合は設定 → お支払いでプランを確認してください。
:::
