...
import { useMarketRoasteries } from '@/shared/api/market-roasteries/queries';
...
export default function MarketRoasteriesPage() {
    ...
    const params = useMemo(() => ({ market, q, limit: PAGE_SIZE, offset }), [market, q, offset]);
    const { data, isPending, isError, error, isFetching } = useMarketRoasteries(params);

    if (isPending && !data) return <QueryLoading label="Loading market roasteries" />;
    if (isError || !data) return <QueryError message={error?.message} />;

    const totalPages = Math.max(1, Math.ceil(data.total / PAGE_SIZE));
    const page = Math.floor(offset / PAGE_SIZE) + 1;
    const marketTotal = Object.values(data.byMarket).reduce((a, b) => a + b, 0);
    ...
    return (
        <>
            ...
            <main className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-4">
                <div className="grid shrink-0 grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-7">
                    ...
                    {MARKETS.filter((m) => m.code !== 'ALL').map((m) => (
                        <KpiCard
                            key={m.code}
                            label={m.label}
                            value={fmtNum(data.byMarket[m.code] ?? 0)}
                ...
                <Panel
                    title="Directory"
                    subtitle={`${fmtNum(data.total)} match · page ${page}/${totalPages}${isFetching ? ' · updating…' : ''}`}
                        ...
                        <tbody>
                            ...
                            {data.items.map((r) => (
                                ...
                                >
                                    ...
                                    <td className="px-3 py-2 tabular-nums">
                                        {r.rating != null ? (
                                            <>
                                                {r.rating}
                                                {r.rating_count != null && (
                                                    <span className="text-[var(--faint)]"> ({r.rating_count})</span>
                                                )}
                                            </>
                                        ) : r.score != null ? (
                                    ...
                                    <td className="px-3 py-2 font-mono text-[10px] text-[var(--muted)]">{r.source}</td>
                                    <td className="px-3 py-2">
                                        {r.maps_url ? (
                                            <a
                                                href={r.maps_url}
