...
import {
  ...
  useDisableCoupon,
  useIssueCoupon,
} from "@/shared/api/coupons/queries";
import type {
  CouponDuration,
  CouponStatus,
  CouponType,
...
export default function CouponsPage() {
  ...
  const labelCls =
    "mb-1 block font-mono text-[10px] uppercase tracking-[0.14em] text-[var(--faint)]";

  return (
    <>
      <TopBar
        title="Coupons"
        subtitle="Polar single-use codes · idle pool → issue → redeem"
      />
                  ...
                  {coupons.map((c) => (
                    ...
                    >
                      <td className="py-2 pr-3">
                        ...
                        >
                          {c.code}
                          <Copy size={11} strokeWidth={1.75} className="text-[var(--faint)]" />
                        </button>
                        {copiedId === c.id && (
