...
export type CreatePolarDiscountInput = {
    ...
    /** Restrict to these product UUIDs. Empty/omit = all products (avoid for coupons). */
    products?: string[];
};
...
export async function createPolarDiscount(input: CreatePolarDiscountInput): Promise<PolarDiscount> {
    ...
    if (input.products != null && input.products.length > 0) {
        body.products = input.products;
    }
