...
import { parseCouponProductScope, productIdsForScope } from '@/shared/lib/polar/productScope';
import { getAdminClient } from '@/shared/lib/supabase/admin';
import type { CouponProductScope, CouponStatus, CouponsListResponse, CreateCouponsInput, PolarCoupon } from './types';
...
function asCoupon(row: CouponRow): PolarCoupon {
    return {
        ...
        product_scope: parseCouponProductScope((row as { product_scope?: unknown }).product_scope),
    };
}
...
export async function listCoupons(): Promise<CouponsListResponse> {
    ...
    const coupons = (data ?? []).map((r) => asCoupon(r as CouponRow));
...
export async function createCouponBatch(input: CreateCouponsInput): Promise<PolarCoupon[]> {
    ...
    const productScope: CouponProductScope = parseCouponProductScope(input.product_scope);
    const products = productIdsForScope(productScope);
    ...
    for (let i = 0; i < count; i++) {
        ...
        let polar: PolarDiscount | undefined;
        let attempts = 0;
        while (attempts < 5) {
            attempts++;
            try {
        ...
        const row = {
            ...
            currency: input.currency ?? 'usd',
            duration: input.duration,
            duration_in_months: input.duration === 'repeating' ? (input.duration_in_months ?? null) : null,
        ...
        const { data, error } = await sb.from('polar_coupons').insert(row).select('*').single();
...
export async function issueCoupon(id?: string): Promise<PolarCoupon> {
    ...
    if (id) {
        const { data: existing, error: fetchErr } = await sb.from('polar_coupons').select('*').eq('id', id).single();
