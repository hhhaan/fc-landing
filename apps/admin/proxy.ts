...
import {
  COUNTRY_HEADER,
  REGION_HEADER,
  resolveRequestGeoFromNext,
} from "@/shared/lib/geo-region";
...
function verify(token: string | undefined): boolean {
  ...
  const secret =
    process.env.ADMIN_SESSION_SECRET || process.env.ADMIN_PASSWORD || "dev";
...
export function proxy(request: NextRequest) {
  ...
  if (!verify(token)) {
    ...
    return withGeoHeaders(request, NextResponse.redirect(url));
  }
