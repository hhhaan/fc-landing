import { POLAR_SERVER } from 'astro:env/server';
import { type BillingMarket, resolveBillingMarket } from './market';
import { POLAR_PRODUCT_IDS, type PolarProductEnvKey } from './polarProducts';

export { type BillingMarket, resolveBillingMarket };
...
export function polarProductId(plan: CheckoutPlan, market: BillingMarket, env: PolarEnv = {}): string | undefined {
    ...
    if (enterprise) {
        if (yearly)
            return (
                envProduct(env, 'POLAR_PRICE_ENTERPRISE_YEARLY_SBX') ??
                envProduct(env, 'POLAR_PRICE_ENTERPRISE_YEARLY_PROD')
            );
        return (
            envProduct(env, 'POLAR_PRICE_ENTERPRISE_MONTHLY_SBX') ??
    ...
    const envKey = `POLAR_${planKey}_PRODUCT_ID_${market}${intervalKey}` as PolarProductEnvKey;
    return envProduct(env, envKey);
}

export function isCheckoutPlanReady(plan: CheckoutPlan, market: BillingMarket, env: PolarEnv = {}): boolean {
    return !!polarProductId(plan, market, env);
}

export function polarServer(): 'sandbox' | 'production' {
    return POLAR_SERVER === 'sandbox' ? 'sandbox' : 'production';
}
